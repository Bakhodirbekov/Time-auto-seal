...
@section('page_title', 'Yangi admin qo\'shish')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-2xl font-bold text-gray-800">Yangi admin qo'shish</h2>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="role" required 
                        ...
                        <option value="moderator" {{ old('role') == 'moderator' ? 'selected' : '' }}>Moderator (Cheklangan huquqlar)</option>
                        @if(auth()->user()->role === 'superadmin')
                            <option value="admin" {{ old('role') == 'admin' ? 'selected' : '' }}>Administrator (To'liq huquqlar)</option>
                        @endif
                    </select>
                    @error('role')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
